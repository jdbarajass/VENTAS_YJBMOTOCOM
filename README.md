...
### Páginas protegidas
Las páginas **Configuración**, **Exportar/Importar**, **Cuentas** y **Rendimiento Vendedores** requieren contraseña para acceder. La sesión desbloquea la página hasta que:
...
| Rol | Acceso |
|-----|--------|
| **Admin** | Todo, incluyendo Configuración, Exportar/Importar, Cuentas y Rendimiento Vendedores, con costos y ganancias reales en todas las vistas |
| **Vendedor** | Sin acceso a Configuración, Exportar/Importar, Cuentas y Rendimiento Vendedores. En Registrar Venta, Ventas del Día, Historial, Vista del Día, Calculadora e Inventario **no ve el costo real de los productos** (se muestra un costo ficticio = costo real × 1.30) ni la ganancia/margen de las ventas (ganancia bruta/neta, comisión %, indicador de ganancia/pérdida, margen %, utilidad real). El "Valor total de inventario" se oculta por completo. Los botones de exportar/imprimir reportes que incluyen costo o ganancia (PDF de Historial, Rentabilidad, Vista del Día, Inventario) no están disponibles. Sí ve las ventas, ingresos totales y cantidades — necesarios para su labor operativa (Mi Cuadre, atención al cliente, recibos) |
...
| F10 | **CRÍTICO — Talla invisible y descuento de stock incorrecto en Registrar Venta** — La Fase 7.1 migró `talla` del nombre del producto (`-T:S`) a una columna separada en `inventario`, pero el formulario de venta nunca se actualizó. Tres consecuencias: (1) el autocomplete mostraba entradas idénticas para el mismo producto en distintas tallas, sin poder distinguirlas; (2) el combo de talla nunca aparecía (buscaba el patrón `-T:XX` en el nombre, que ya no existe); (3) `decrementar_cantidad` sin filtro de talla ejecutaba `UPDATE inventario ... WHERE LOWER(producto) = LOWER(?)` sin `AND talla = ?`, descontando stock de **todas las tallas** simultáneamente al vender una sola. Corregido con tres cambios: el autocomplete ahora muestra `"NOMBRE — TALLA"` con un mapa interno `{display → Producto}` para recuperar el objeto exacto al seleccionar; `_aplicar_producto` usa `producto.talla` en vez del regex; `decrementar_cantidad` e `incrementar_cantidad` reciben un parámetro `talla=""` y, cuando está presente, agregan `AND LOWER(talla) = LOWER(?)` al WHERE para afectar solo el registro correcto; `guardar_carrito` extrae la talla de cada línea del carrito y la pasa al decremento — archivos: `database/inventario_repo.py`, `ui/venta_form.py`, `controllers/venta_controller.py` |
...
| F11 | **Costo real y ganancia ocultos para el rol Vendedor en toda la app** — antes solo el Dashboard tenía una vista resumida para vendedor (F7.8); el resto de pantallas (Registrar Venta, Editar Venta, Ventas del Día, Historial, Vista del Día, Calculadora, Inventario) seguían mostrando el costo real y la ganancia/margen de cada venta sin ninguna restricción. Se agregó `utils/permisos.py` (`es_vendedor`, `costo_mostrado`) y se propagó el rol a los paneles/diálogos correspondientes: el vendedor ve un costo ficticio = costo real × 1.30 (el costo real sigue siendo el que se guarda en la BD, desacoplado del valor mostrado — ver `_LineaProducto._costo_real`/`_costo_conocido` en `ui/venta_form.py`) y no ve ganancia bruta/neta, comisión %, margen %, ni el indicador de ganancia/pérdida en ningún lado; el "Valor total de inventario" se oculta por completo (a diferencia del costo unitario, que sí lleva el markup); los botones de exportar/imprimir reportes con costo o ganancia (Historial, Rentabilidad, Vista del Día, Inventario) se ocultan para ese rol en vez de reescribir los generadores de PDF. Ventas e ingresos totales siguen visibles (necesarios para el trabajo diario del vendedor). De paso se corrigieron dos huecos de permisos encontrados en la auditoría: "Rendimiento Vendedores" solo ocultaba el botón de navegación sin pedir contraseña (ahora protegida igual que Configuración/Exportar/Cuentas), y el Dashboard quedaba con el rol de la sesión anterior si se cerraba sesión sin reiniciar la app (ahora se reconstruye al cambiar de usuario). Verificado con smoke tests que construyen la ventana con ambos roles y confirman, con valores numéricos reales, que cada dato se oculta o se muestra correctamente y que el costo guardado en BD nunca es el inflado |
