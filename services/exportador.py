...
def exportar_ventas_dia(ventas: list[Venta], fecha: date, ruta: Path) -> None:
    ...
    # ---- Encabezados ----
    headers = [
        "#", "Fecha", "Producto", "Costo", "Precio venta",
    ...
    for i, v in enumerate(ventas, start=1):
        ...
        # Color ganancia neta
        cell_neta = ws.cell(row=row, column=8)
    ...
    # ---- Anchos de columna ----
    anchos = [5, 12, 30, 15, 16, 14, 14, 15, 28]
...
def exportar_ventas_mes(ventas: list[Venta], año: int, mes: int, ruta: Path) -> None:
    ...
    for i, v in enumerate(ventas, start=1):
        ws.append([
            i, fecha_corta(v.fecha), v.producto,
